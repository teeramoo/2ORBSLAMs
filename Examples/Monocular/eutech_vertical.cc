...
int main(int argc, char **argv)
{
   	if (argc != 7)
    {
        cerr << endl << "Usage: " << argv[0]
                << " <path_to_vocabulary> <path_to_settings> <path_to_video> <path_to_imu> <path_to_output_folder> <frames_to_skip>"
                << endl;
        return 1;
    }
    
    string strVideoFile = string(argv[3]);
    string strOutputPath = string(argv[5]);
    int skip = atoi(argv[6]);
    //string strFrameDataFile = string(argv[4]);
    string strIMUDataFile = string(argv[4]);
    ...
    ORB_SLAM2::System SLAM(argv[1],argv[2],ORB_SLAM2::System::MONOCULAR,strOutputPath,true);
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        //cv::Mat roi(im, cv::Rect(0, 0, im.cols, im.rows / 2));
		//roi = cv::Scalar(0,0,0);	
        ...
        //cout << "IMU time: " << setprecision(16) << tempIMU << endl;
        //cout << "Frame time: " << setprecision(16) << tempFrame << endl;
        //cout << "Time diff: " << setprecision(16) << tempFrame - tempIMU << endl;
        //while((tframe - vIMUPositions[nIMUc].timestamp) >= 0)
        
        
        for(int k = 0; k < 4; k++)
        {
        	//cout << nIMUc << " IMU time: "<< setprecision(16) << tempIMU << endl;
        	//cout << nIMUc << " Frame time: " << setprecision(16) <<  tempFrame << endl;
        ...
        SLAM.TrackMonoVI(im, vimuData, tempFrame);
