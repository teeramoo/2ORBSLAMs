...
//-------------------------------------------------------------------------------------------
    ...
    static void transformPt(cv::Mat &P, cv::Mat &T, cv::Mat &Pp)
    {
        assert(P.type() == CV_32F);
        assert(T.type() == CV_32F);
        if (Pp.empty()) Pp = cv::Mat(1, 3, CV_32F);
        assert(Pp.type() == CV_32F);
        float X = P.at<float>(0, 0);
        float Y = P.at<float>(1, 0);
        float Z = P.at<float>(2, 0);
        Pp.at<float>(0, 0) = T.at<float>(0, 0) * X + T.at<float>(0, 1) * Y + T.at<float>(0, 2) * Z + T.at<float>(0, 3);
        Pp.at<float>(1, 0) = T.at<float>(1, 0) * X + T.at<float>(1, 1) * Y + T.at<float>(1, 2) * Z + T.at<float>(1, 3);
        Pp.at<float>(2, 0) = T.at<float>(2, 0) * X + T.at<float>(2, 1) * Y + T.at<float>(2, 2) * Z + T.at<float>(2, 3);
    }
...
//            cout << "Check if it's an error in mForceFrame3" << endl;
//                cv::Mat Tcl = mCurrentFrame.mTcw * mLastFrame.mTcw.inv();
  //          cv::Mat Tcl = mCurrentFrame.mTcw * LastTwc;//--> fix inversion matrix
//            cout << "Check if it's an error in mForceFrame4" << endl;
 //               mCurrentFrame.SetPose(Tcl * mLastFrame.mTcw);
//            cout << "Check if it's an error in mForceFrame5" << endl;
...
#endif
    Optimizer::PoseOptimization(&mCurrentFrame, true,4.0);
...
void Tracking::SetForceFrameMat(cv::Mat &_mForcedFrame)
    {
        mForcedFrame.release();
        mForcedFrame = _mForcedFrame;
...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
                   Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor,ConfigParam* pParams, bool _bDebug):
...
void Tracking::MonocularInitialization()
{
    ...
    if(iFrameNo == forcedInitFrame  && isSecondSLAM)
    {
        bForceInit = true;
...
// Added on 2/1/2018
//    cv::Mat Twc1 = mInitialFrame.mTcw.inv();
// End of added on 2/1/2018
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        // Added on 2/1/2018
        // transformPt(worldPos, Twc1, worldPos);
        // End of added on 2/1/2018
...
bool Tracking::TrackLocalMap()
{
  ...
  //      Optimizer::PoseOptimization(&mCurrentFrame);
    if(!isSecondSLAM)
        Optimizer::PoseOptimization(&mCurrentFrame,false,1.0);
    else
        Optimizer::PoseOptimization(&mCurrentFrame, true,4.0);
...
void Tracking::SaveMapPoints()
{
    ...
    ss1 << _sSaveFolder << "MapPoints_with_keyframes_" << _iFileIndex << ".pcd";

	string sFileName = ss.str();
    string sFileName1 = ss1.str();
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        f << setprecision(7) << pos.at<float>(0) << setprecision(7) << " " << pos.at<float>(1) << " " << pos.at<float>(2) << " 16711746"<< flush;
        f << endl;
    }
    ...
    f << "TYPE F F F F\n" << flush;
    f << "COUNT 1 1 1 1\n" << flush;
    f << "WIDTH " << iWCount + vpKFs.size() << "\n" << flush;
    f << "HEIGHT 1\n" << flush;
    f << "VIEWPOINT 0 0 0 1 0 0 0\n" << flush;
    f << "POINTS " << iWCount + vpKFs.size() << "\n" << flush;
    f << "DATA ascii\n" << flush;
    f << endl;
    f << fixed;
    ...
    f.close();
    cout << endl << "Map Points saved with " << iWCount << " points and "<< vpKFs.size() << " keyframes." << endl;
