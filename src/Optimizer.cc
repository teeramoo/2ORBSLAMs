...
//-------------------------------------------------------------------------------------------
    ...
    int Optimizer::PoseOptimization(Frame *pFrame, bool isSecondSLAM, float pixelError)
    {
        ...
        //Added by Teera
        cv::Mat mCurrentMat = pFrame->mTcw.clone();

        //End of added by Teera
...
//    const float chi2Mono[4]={5.991,5.991,5.991,5.991};
        ...
        pose2.at<float>(0,0) = pose.at<float>(0,0);
        pose2.at<float>(0,1) = pose.at<float>(0,1);
        pose2.at<float>(0,2) = pose.at<float>(0,2);
        pose2.at<float>(1,0) = pose.at<float>(1,0);
        pose2.at<float>(1,1) = pose.at<float>(1,1);
        pose2.at<float>(1,2) = pose.at<float>(1,2);
        pose2.at<float>(2,0) = pose.at<float>(2,0);
        pose2.at<float>(2,1) = pose.at<float>(2,1);
        pose2.at<float>(2,2) = pose.at<float>(2,2);

        pose2.at<float>(0,3) = mCurrentMat.at<float>(0,3);
        pose2.at<float>(1,3) = mCurrentMat.at<float>(1,3);
        pose2.at<float>(2,3) = mCurrentMat.at<float>(2,3);

//        pose(cv::Rect(0,0,3,3)).copyTo(pose2(cv::Rect(0,0,3,3)));
//        mCurrentMat(cv::Rect(3,0,1,3)).copyTo(pose2(cv::Rect(3,0,1,3)));
        if(!isSecondSLAM)
        {
            pFrame->SetPose(pose);
        ...
        {
            pFrame->SetPose(pose);// pFrame->SetPose(mCurrentMat);// pFrame->SetPose(pose2);
      //      pFrame->SetPose(mCurrentMat);
      //      cout << "output pose is " << mCurrentMat << endl;
        }
